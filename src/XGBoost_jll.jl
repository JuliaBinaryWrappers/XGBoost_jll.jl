...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import XGBoost_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("XGBoost_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "XGBoost_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"XGBoost", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "XGBoost_jll", "src", "XGBoost_jll.jl"))
        @info("XGBoost_ll dev'ed out to /depot/dev/XGBoost_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("a5c6f535-4255-5ca2-a466-0e519f119c46"))
platforms = [Pkg.Artifacts.unpack_platform(e, "XGBoost", artifacts_toml) for e in artifacts["XGBoost"]]
...
if best_platform === nothing
    @debug("Unable to load XGBoost; unsupported platform $(triplet(platform_key_abi()))")
else
